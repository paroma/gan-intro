...
class DataDistribution(object):
    ...
    def sample(self, N):
        #samples1 = np.random.normal(0., 0.005, int(3*N/4))
        samples1 = np.random.normal(self.mu2, self.sigma2, int(3*N/4))
        #SL Change: making distribution "bimodal"
        samples2 = np.random.normal(self.mu2, self.sigma2, int(N/4))
    ...
    def train(self):
        with tf.Session() as session:
            ...
            for step in xrange(num_pretrain_steps):
                ...
                #Change from SL, train with "correct" labels
                labels = norm.pdf(d, loc=self.data.mu1, scale=self.data.sigma1)
...
def train_SL(num_steps=500, batch_size=12, minibatch=False, log_every=10, anim=None):
    model = GAN(
        DataDistribution(),
